...
(function() {
  ...
  ClassifyPage = (function(_super) {
    ...
    ClassifyPage.prototype.onActivate = function(e) {
      ...
      if ((this.classification != null) && this.targetSubjectID) {
        if (this.classification.subject.zooniverse_id !== this.targetSubjectID) {
      ...
      if (User.current != null) {
        ...
        $.when(project, talkUser).then((function(_this) {
          return function(project, talkUser) {
            var projectRoles, _ref;
            projectRoles = (_ref = talkUser.roles) != null ? _ref[project.id] : void 0;
            return result.resolve((projectRoles != null) && __indexOf.call(projectRoles, 'scientist') >= 0);
          };
        })(this));
      } else {
    ...
    ClassifyPage.prototype.getNextSubject = function() {
      ...
      if (this.targetSubjectID) {
        if (this.targetSubjectID !== ((_ref = this.classification) != null ? _ref.subject.zooniverse_id : void 0)) {
          request = Api.current.get("/projects/" + Api.current.project + "/subjects/" + this.targetSubjectID);
          ...
          return request.fail(function() {
            return alert("There's no subject with the ID " + this.targetSubjectID + ".");
          });
        }
      } else {
    ...
    ClassifyPage.prototype.sendClassification = function() {
      ...
      if (this.targetSubjectID) {
        this.isUserScientist().then((function(_this) {
          return function(theyAre) {
            if (theyAre) {
              return ClassifyPage.__super__.sendClassification.apply(_this, arguments);
            } else {
              return alert('Sorry, classifying specific subjects is only available to scientsts!');
            }
          };
        })(this));
      } else {
